...
* v4.1.0 - ~~`LESS/CSS`~~ ~~`HTML`~~ `JS`
	* According to the previous guy, the tabcordion accordion should work with a single item and toggle open/close on click, but it doesn't! This change enables a toggle for tabcordion accordion's with a single item.
